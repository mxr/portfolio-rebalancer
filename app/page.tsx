...
const parseSortState = (value: string | null): SortState | null => {
  ...
  if ((key === "ticker" || key === "current" || key === "target" || key === "amount") && (direction === "asc" || direction === "desc")) {
...
function HomeContent() {
  ...
  const [csvPositions, setCsvPositions] = useState<FidelityCsvPosition[] | null>(null);
  ...
  const getSortIndicator = (key: SortKey) => (sortState.key !== key ? "↕" : sortState.direction === "asc" ? "▲" : "▼");
  ...
  const handleTabAddRow = (event: React.KeyboardEvent<HTMLInputElement>, isLastRow: boolean) => {
  ...
  const handleCsvUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      setImportError(null);
      const { cashCurrent, positions, pendingActivity: pending } = parseFidelityCsv(text);
      ...
      <main className="relative mx-auto flex w-full max-w-6xl flex-col gap-10 px-6 py-16 sm:px-10 lg:px-12">
        <header className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-3">
            <h1 className="text-4xl font-semibold tracking-tight text-[#161515] sm:text-5xl">Rebalance your portfolio.</h1>
            <p className="w-full text-base text-[#4a4037] sm:text-lg">
              Enter tickers, amounts, and target allocation percentages. The calculator updates instantly with how much to sell or buy per
              holding to reach the target allocation. A trade summary is shown at the bottom. You can also upload a .csv of your positions
              downloaded from Fidelity (this tool is not affiliated with Fidelity in any way) to auto-fill holdings and show estimated
              gain/loss for sales. Everything runs locally in your browser - data never leaves your machine. State is stored in the URL so
              you can save the page for later or share it with others.
            </p>
          </div>
        </header>
        ...
        <section className="rounded-3xl border border-[#e7d7c8] bg-white/80 p-6 shadow-[0_20px_60px_rgba(120,96,77,0.12)] backdrop-blur">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <h2 className="text-2xl font-semibold text-[#181716]">Allocation Inputs</h2>
              ...
              {importError ? <p className="mt-2 text-xs font-semibold uppercase tracking-[0.2em] text-[#b44b43]">{importError}</p> : null}
            </div>
          ...
          <div className="mt-6">
            <div className="hidden items-center gap-4 border-b border-[#f2e8dd] pb-3 text-xs font-semibold uppercase tracking-[0.3em] text-[#8c7b6c] sm:grid sm:grid-cols-5">
              <button type="button" onClick={() => updateSort("ticker")} className="flex items-center gap-2">
                Ticker
                <span className="text-[10px]">{getSortIndicator("ticker")}</span>
              </button>
              <button type="button" onClick={() => updateSort("current")} className="flex items-center gap-2">
                Current
                <span className="text-[10px]">{getSortIndicator("current")}</span>
              </button>
              <button type="button" onClick={() => updateSort("target")} className="flex items-center gap-2">
                Target
                <span className="text-[10px]">{getSortIndicator("target")}</span>
              </button>
              <button type="button" onClick={() => updateSort("amount")} className="flex items-center gap-2">
                Trade
                ...
                return (
                  ...
                  >
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          (() => {
                            ...
                            const hadIllegalChars = sanitized !== raw.toUpperCase();
                            if (hadIllegalChars) {
                              showInvalidHint(row.id, "ticker", "Ticker allows A-Z and 0-9 only.");
                            }
                        ...
                        className={`h-12 w-full rounded-xl border px-3 text-sm font-medium outline-none transition focus:border-[#c9a888] focus:ring-2 focus:ring-[#edc9a6]/60 ${
                          isCash ? "border-[#e6d7c7] bg-[#fff7ed] text-[#7a6757]" : "border-[#e6d7c7] bg-[#fefbf7] text-[#1d1b18]"
                      ...
                      {isCash && pendingActivity !== null ? (
                        <span className="group absolute right-3 top-1/2 z-10 flex h-6 w-6 -translate-y-1/2 items-center justify-center rounded-full border border-[#d6c5b3] bg-[#fff7ed] text-[10px] font-semibold text-[#7a6757]">
                          ...
                          <span className="pointer-events-none absolute left-1/2 top-full z-20 mt-2 w-max max-w-xs -translate-x-1/2 whitespace-nowrap rounded-lg border border-[#e6d7c7] bg-white px-3 py-2 text-[11px] font-medium text-[#4a4037] opacity-0 shadow-[0_8px_20px_rgba(30,27,23,0.15)] transition group-hover:opacity-100">
                            Includes pending activity: {formatCurrency(pendingActivity)}
                          </span>
                        </span>
                      ) : null}
                      {invalidHint?.id === row.id && invalidHint.field === "ticker" ? (
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          (() => {
                            ...
                            if (sanitized !== raw) {
                              showInvalidHint(row.id, "current", "Amount must be a number with up to 2 decimals.");
                            }
                      ...
                      />
                      {invalidHint?.id === row.id && invalidHint.field === "current" ? (
                        <div className="pointer-events-none absolute left-0 top-full z-20 mt-1 rounded-lg border border-[#e6d7c7] bg-white px-3 py-1 text-[11px] font-medium text-[#7d3d37] shadow-[0_8px_20px_rgba(30,27,23,0.15)]">
                          {invalidHint.message}
                        </div>
                      ) : null}
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            (() => {
                              const raw = event.target.value;
                              const sanitized = sanitizeTargetPercentInput(raw);
                              if (sanitized !== raw) {
                                showInvalidHint(row.id, "target", "Percent must be a number with up to 2 decimals.");
                              }
                        ...
                        />
                        {invalidHint?.id === row.id && invalidHint.field === "target" ? (
                          <div className="pointer-events-none absolute left-0 top-full z-20 mt-1 rounded-lg border border-[#e6d7c7] bg-white px-3 py-1 text-[11px] font-medium text-[#7d3d37] shadow-[0_8px_20px_rgba(30,27,23,0.15)]">
                            {invalidHint.message}
                          </div>
                        ) : null}
                    ...
                    <div
                      className={`flex h-12 items-center gap-2 rounded-xl border border-dashed px-3 text-base font-semibold ${
                        isCash ? "border-[#e7d8c7] bg-[#fff7ed] text-[#4a4037]" : "border-[#d6c5b3] bg-[#fbf6ef] text-[#2b241f]"
                      }`}
                    >
                      <span className="text-xs uppercase tracking-[0.2em] text-[#7a6757]">{actionLabel}</span>
                      <span>{formatCurrency(Math.abs(delta))}</span>
                    </div>
                    <div className="flex h-12 items-center justify-end">
                      {isCash ? (
                        <span className="text-xs uppercase tracking-[0.2em] text-[#b39d8a]">Zero out</span>
                      ) : (
                        <button
                          type="button"
                        ...
                        >
                          ...
                          >
                            ...
                            <path d="M7.34491 2.4C7.67506 1.7 8.37973 1.3 9.15396 1.3H14.846C15.6203 1.3 16.3249 1.7 16.6551 2.4L18.6 6H5.4L7.34491 2.4Z" />
                            <path d="M2 6H22" />
                            <path d="M10 11V16" />
                            <path d="M14 11V16" />
                          </svg>
        ...
        <section className="rounded-3xl border border-[#e7d7c8] bg-white/80 p-6 shadow-[0_20px_60px_rgba(120,96,77,0.12)] backdrop-blur">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <h2 className="text-2xl font-semibold text-[#181716]">Trade Summary</h2>
              <p className="text-sm text-[#5b5148]">Suggested trades based on your target allocations.</p>
            </div>
          </div>

          <div className="mt-6 grid gap-6 md:grid-cols-2">
            <div className="rounded-2xl border border-[#f2e8dd] bg-[#fff8ef] p-4">
              <p className="text-xs uppercase tracking-[0.3em] text-[#8a7768]">Sell</p>
              {tradeSummary.sells.length === 0 ? (
                <p className="mt-3 text-sm text-[#7a6a5d]">Nothing to sell.</p>
              ) : (
            ...
            <div className="rounded-2xl border border-[#f2e8dd] bg-[#f3f7f3] p-4">
              <p className="text-xs uppercase tracking-[0.3em] text-[#7b8a76]">Buy</p>
              {tradeSummary.buys.length === 0 ? (
                <p className="mt-3 text-sm text-[#6c7a66]">Nothing to buy.</p>
              ) : (
                <div className="mt-3 flex flex-col gap-2">
                  {tradeSummary.buys.map((item) => (
                    ...
                    >
                      <span className="font-semibold">{item.ticker}</span>
                      <span className="truncate sm:text-right">{formatCurrency(item.amount)}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          {csvPositions ? (
            <p className="mt-4 text-xs text-[#6c6258]">
              Disclaimer: Estimated gain/loss assumes all lots for each security have the same cost basis. Actual gain/loss depends on
              long-term vs short-term treatment and the specific cost basis of each lot.
            </p>
          ) : null}
        </section>
      </main>
